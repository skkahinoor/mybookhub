                ...
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="text-muted mb-0">Placed on {{ $orderDetails->created_at->format('F d, Y h:i A') }}</p>
                        </div>
                        <a href="{{ route('user.orders.index') }}" class="btn btn-secondary" style="background-color: rgb(63, 61, 61); color: white;">
                        ...
                        <div class="card-body">
                            ...
                            <div class="order-timeline" style="position: relative; padding-left: 30px;">
                                <div class="timeline-item" style="position: relative; padding-bottom: 20px;">
                                    <div style="position: absolute; left: -24px; top: 4px; width: 16px; height: 16px; background: #28a745; border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 0 2px #28a745;"></div>
                                    <div>
                                        <strong>Order Placed</strong>
                                        <p class="text-muted mb-0 small">{{ $orderDetails->created_at->format('M d, Y h:i A') }}</p>
                                    </div>
                                </div>
                                @if(strpos($status, 'shipped') !== false || strpos($status, 'delivered') !== false)
                                <div class="timeline-item" style="position: relative; padding-bottom: 20px;">
                                    <div style="position: absolute; left: -24px; top: 4; width: 16px; height: 16px; background: #28a745; border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 0 2px #28a745;"></div>
                                    <div>
                                        <strong>Shipped</strong>
                                        <p class="text-muted mb-0 small">{{ $orderDetails->updated_at->format('M d, Y h:i A') }}</p>
                                    </div>
                                </div>
                                @endif
                                ...
                                @else
                                <div class="timeline-item" style="position: relative;">
                                    <div style="position: absolute; left: -24px; top: 4; width: 16px; height: 16px; background: #e0e0e0; border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 0 2px #e0e0e0;"></div>
                                    <div>
                            ...
                            @foreach ($orderDetails->orders_products as $product)
                                <div class="d-flex align-items-center mb-4 pb-4" style="border-bottom: 1px solid #e5e5e5;">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <p class="text-muted mb-0">Price: ₹{{ number_format($product->product_price, 2) }}</p>
                                        @if($product->courier_name)
                                            <p class="text-info mb-0 mt-2">
                                                <small>Courier: {{ $product->courier_name }} | Tracking: {{ $product->tracking_number }}</small>
                                            </p>
                                        @endif
                                    </div>
                                    <div class="text-right">
                                        <h5 class="mb-0">₹{{ number_format($product->product_price * $product->product_qty, 2) }}</h5>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="card mb-4" style="position: sticky; top: 20px;">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                <span>Subtotal:</span>
                                <span>₹{{ number_format($orderDetails->grand_total - $orderDetails->shipping_charges - ($orderDetails->coupon_amount ?? 0), 2) }}</span>
                            </div>
                            @if($orderDetails->coupon_code)
                            <div class="d-flex justify-content-between mb-2">
                                <span>Coupon ({{ $orderDetails->coupon_code }}):</span>
                                <span class="text-success">-₹{{ number_format($orderDetails->coupon_amount ?? 0, 2) }}</span>
                            </div>
                            @endif
                            <div class="d-flex justify-content-between mb-2">
                                <span>Shipping:</span>
                            ...
                            <div class="d-flex justify-content-between">
                                <strong>Total:</strong>
                                <strong style="font-size: 18px; color: #28a745;">₹{{ number_format($orderDetails->grand_total, 2) }}</strong>
                            </div>
                            <hr>
                            <div class="mb-2">
